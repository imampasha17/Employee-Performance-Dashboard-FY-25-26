...
export function Dashboard({
  ...
  const { metrics, totalRevenue, totalCount, uniqueEmployees, uniqueLocations, schemeData } =
    useMemo(() => {
      const m = {
  ...
  const handleLocationClick = (locationName: string) => {
    ...
    const locData = filteredData.filter(
      (d) => (d.location || '').toLowerCase() === locationName.toLowerCase(),
    );
    ...
    setDetailData({
      ...
      totalDueCount: new Set(
        locData
          .filter((d) => (d.totalDue || 0) > 0)
          .map((d) => d.profileNo || d.customerName || d.id)
          .filter(Boolean),
      ).size,
      collectionCustomerCount: new Set(
        locData
          .filter((d) => (d.collectionReceivedValue || 0) > 0)
          .map((d) => d.profileNo || d.customerName || d.id)
          .filter(Boolean),
      ).size,
      schemes: {
        count11Plus1: locData
          .filter((d) => normalizeSchemeName(d.schemeType || '') === '11+1')
          .reduce((sum, d) => sum + (d.enrolmentCount || 0), 0),
        count11Plus2: locData
          .filter((d) => normalizeSchemeName(d.schemeType || '') === '11+2')
          .reduce((sum, d) => sum + (d.enrolmentCount || 0), 0),
        countGpRateShield: locData
          .filter((d) => normalizeSchemeName(d.schemeType || '') === 'Rate_Shield')
          .reduce((sum, d) => sum + (d.enrolmentCount || 0), 0),
        countOnePay: locData
          .filter((d) => normalizeSchemeName(d.schemeType || '') === 'One_Pay')
          .reduce((sum, d) => sum + (d.enrolmentCount || 0), 0),
      },
    });
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            >
              ...
              <div>
                <span className="text-xl sm:text-2xl font-black text-slate-900 tracking-tighter block leading-none">
                  PerformanceHub
                </span>
                <span className="text-[10px] font-bold text-blue-600 uppercase tracking-[0.2em] leading-none mt-1 block">
          ...
          {activeTab === 'admin' && user.role === 'admin' ? (
            ...
            >
                ...
                <div className="space-y-4 flex-1">
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    ...
                    <div className="relative w-full sm:max-w-xs group">
                      ...
                      >
                        <option value="">
                          {selectedUserId ? "User's Locations" : 'All Locations'}
                        </option>
              ...
              <div className="space-y-12 pb-10">
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3 sm:gap-4">
                    <StatsCard
                      title="Enrolment Count"
                      value={formatNumber(metrics.enrolment.count)}
                      icon={FileSpreadsheet}
                      iconClassName="bg-blue-50 text-blue-600"
                      description="Total accounts enrolled"
                    />
                    <StatsCard
                      title="Enrolment Value"
                      value={formatCurrency(metrics.enrolment.value)}
                      icon={IndianRupee}
                      iconClassName="bg-blue-50 text-blue-600"
                      description="Gross scheme enrolment"
                    />
                    <StatsCard
                      title="Re-Enrollment Count"
                      value={formatNumber(metrics.reEnrolment.count)}
                      icon={RefreshCw}
                      iconClassName="bg-cyan-50 text-cyan-600"
                      description="Renewed accounts count"
                    />
                    <StatsCard
                      title="Re-Enrollment Value"
                      value={formatCurrency(metrics.reEnrolment.value)}
                      icon={IndianRupee}
                      iconClassName="bg-cyan-50 text-cyan-600"
                      description="Sum of renewal amounts"
                    />
                    <StatsCard
                      title="UP Sale"
                      value={formatCurrency(metrics.upSale.value)}
                      subValue={`${formatNumber(metrics.upSale.count)} Up Sales`}
                      icon={ArrowUpRight}
                    ...
                    <StatsCard
                      title="Expected Inst."
                      value={formatCurrency(metrics.expected.value)}
                      icon={Calendar}
                      iconClassName="bg-indigo-50 text-indigo-600"
                      description="Expected monthly total"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-3 sm:gap-4">
                    <StatsCard
                      ...
                      value={formatCurrency(metrics.collectionRcvd.value)}
                      icon={TrendingUp}
                      iconClassName="bg-emerald-50 text-emerald-600"
                      description="Sum of Current Received Amount"
                    />
                    <StatsCard
                    ...
                    />
                    <StatsCard
                      title="Payment vs Overdue"
                      value={formatCurrency(metrics.paymentOverdue.value)}
                      icon={IndianRupee}
                      iconClassName="bg-emerald-50 text-emerald-600"
                      description="Payment Received Against Over Due"
                    />
                    <StatsCard
                      title="Current Due Collection"
                      value={formatCurrency(metrics.currentDueColl.value)}
                      icon={IndianRupee}
                      iconClassName="bg-emerald-50 text-emerald-600"
                      description="Current Due Against Collection"
                    />
                  </div>
                </div>
                {/* 3. Outstanding & Dues */}
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3 sm:gap-4">
                    <StatsCard
                      title="Total Due Value"
                      value={formatCurrency(metrics.totalDue.value)}
                      icon={AlertCircle}
                    ...
                    <StatsCard
                      ...
                      icon={Users}
                      iconClassName="bg-rose-50 text-rose-600"
                      description="Total entries O/S"
                    />
                    <StatsCard
                      title="Overdue Pending Amt"
                      value={formatCurrency(metrics.overdueAmt.value)}
                      subValue={`${formatNumber(metrics.overdue.count)} Pending`}
                      icon={AlertCircle}
                      iconClassName="bg-rose-50 text-rose-600"
                      description="Total Overdue Pending Amount"
                    />
                    <StatsCard
                      title="Current Due Apr-26"
                      value={formatCurrency(metrics.currentDueAmt.value)}
                      subValue={`${formatNumber(metrics.currentDue.count)} Dues`}
                      icon={Calendar}
                      iconClassName="bg-rose-50 text-rose-600"
                      description="Current month dues"
                    />
                    <StatsCard
                      title="Paid Cust Count"
                      value={formatNumber(metrics.paidCustomers.count)}
                      icon={Users}
                      iconClassName="bg-rose-50 text-rose-600"
                      description="Total paying customers"
                    />
                    <StatsCard
                      title="Expected Inst Amount"
                      value={formatCurrency(metrics.expected.value)}
                      icon={TrendingUp}
                      iconClassName="bg-rose-50 text-rose-600"
                      description="Total expected installments"
                    />
                    <StatsCard
                      title="Foreclosed Count"
                      value={formatNumber(metrics.forclosed.count)}
                      icon={Trash2}
                      iconClassName="bg-slate-100 text-slate-600"
                      description="Total Foreclosed Accounts"
                    />
                    <StatsCard
                      title="Foreclosed Amount"
                      value={formatCurrency(metrics.forclosedAmt.value)}
                      icon={IndianRupee}
                      iconClassName="bg-slate-100 text-slate-600"
                      description="Total Foreclosed Amount"
                    />
                  </div>
                </div>{' '}
                {/* 4. Strategic Metrics */}
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    <StatsCard
                      title="Actual Redemption"
                      value={formatCurrency(metrics.redemption.actual)}
                      icon={Trophy}
                      iconClassName="bg-violet-50 text-violet-600"
                      description="Realized redemptions"
                    />
                    <StatsCard
                      title="Pending Redemption"
                      value={formatCurrency(metrics.redemption.pending)}
                      icon={TrendingUp}
                      iconClassName="bg-violet-50 text-violet-600"
              ...
              <SchemePerformance
                ...
                title={
                  selectedLocation
                    ? `Scheme Performance: ${selectedLocation}`
                ...
                <div className="text-center space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <p className="text-slate-500 text-lg max-w-md mx-auto leading-relaxed">
                      Drag & drop your employee performance CSV files here or click to browse. Get
