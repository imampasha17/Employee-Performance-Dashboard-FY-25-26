    ...
    <div className="bg-white rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 border border-slate-100 shadow-xl shadow-slate-200/40">
      <div className="flex items-center justify-between mb-6 sm:mb-8">
        <div>
          ...
          <p className="text-xs sm:text-sm font-medium text-slate-500 mt-1">Breakdown by scheme type and enrollment value</p>
        </div>
        ...
        {data.map((scheme, idx) => (
          ...
          >
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <span className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-[0.15em]">{scheme.name || "Unknown"}</span>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">{formatNumber(scheme.count)}</span>
                  <span className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest">Units</span>
                </div>
                <div className="flex items-center gap-1.5 mt-1 text-blue-600">
                  <IndianRupee className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                  <span className="text-xs sm:text-sm font-black tracking-tight">{formatCurrency(scheme.value)}</span>
                </div>
              </div>
